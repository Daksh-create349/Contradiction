    ...
    <header className="sticky top-0 z-30 bg-background/95 text-foreground backdrop-blur-xl">
      ...
      <div className="site-container">
        <div className="flex h-20 items-center justify-between gap-6">
          <Link href="/" className="flex shrink-0 items-center" aria-label="Contradiction home" onClick={() => setMenuOpen(false)}>
            <Brand />
          </Link>
          <nav aria-label="Main navigation" className="hidden items-center gap-7 xl:gap-9 lg:flex">
            {navigation.map((item) => (
          ...
          <div className="flex shrink-0 items-center gap-4 sm:gap-5">
            <a
              ...
              className="hidden items-center gap-1.5 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-800 transition-opacity hover:opacity-80 dark:text-emerald-300 md:inline-flex shrink-0"
              aria-label="Glama 100/100 score (opens in a new tab)"
            >
              <span className="size-1.5 rounded-full bg-emerald-500" aria-hidden="true" />
              <span>Glama 100/100</span>
            </a>
            <a href={repositoryUrl} target="_blank" rel="noopener noreferrer" className="hidden items-center gap-2 text-sm font-medium text-muted-foreground transition-opacity hover:opacity-80 sm:inline-flex shrink-0" aria-label="View GitHub repository (opens in a new tab)">
              <Image src="/logos/github.svg" alt="" width={18} height={18} />
              GitHub
              <ArrowUpRight className="size-3.5 text-muted-foreground" aria-hidden="true" />
            </a>
            <a href="/#developers" className={cn(buttonVariants({ size: 'lg' }), 'hidden h-10 px-4 sm:inline-flex shrink-0')}>
              Get started
              <ArrowUpRight data-icon="inline-end" />
            </a>
            <Button ref={menuButtonRef} variant="ghost" size="icon-lg" className="shrink-0 lg:hidden" aria-expanded={menuOpen} aria-controls="mobile-navigation" aria-label={menuOpen ? 'Close navigation' : 'Open navigation'} onClick={() => setMenuOpen((open) => !open)}>
